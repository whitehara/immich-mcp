...
async def test_list_people_defaults(registered):
    ...
    await get_fn(mcp, "immich.people.list")()
...
async def test_list_people_with_hidden(registered):
    ...
    await get_fn(mcp, "immich.people.list")(with_hidden=True)
...
async def test_get_person(registered):
    ...
    result = await get_fn(mcp, "immich.people.get")("p1")
...
async def test_update_person_name(registered):
    ...
    await get_fn(mcp, "immich.people.update")("p1", name="Alice")
...
async def test_update_person_hide(registered):
    ...
    await get_fn(mcp, "immich.people.update")("p1", is_hidden=True)
...
async def test_merge_people(registered):
    ...
    await get_fn(mcp, "immich.people.merge")("target-id", "source-id")
...
def test_merge_is_destructive(registered):
    mcp, _ = registered
    ann = get_annotations(mcp, "immich.people.merge")
...
async def test_statistics(registered):
    ...
    result = await get_fn(mcp, "immich.people.statistics")("p1")
...
def test_list_is_readonly(registered):
    mcp, _ = registered
    ann = get_annotations(mcp, "immich.people.list")
