...
def register(mcp: FastMCP) -> None:
    @mcp.tool(
        ...
        annotations=ToolAnnotations(title="Ping Server", readOnlyHint=True, idempotentHint=True),
    )
    async def ping() -> dict:
        client = get_client()
    ...
    @mcp.tool(
        ...
        annotations=ToolAnnotations(title="Server Capabilities", readOnlyHint=True, idempotentHint=True),
    )
